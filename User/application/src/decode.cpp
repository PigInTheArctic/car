...
void Decode::can_decode(void)
{
   ...
   for (uint8_t i = 0; i < motor_num; i++)
   {
      ...
      case 0:
         ...
         chassis.speed_lf_rcv = math::turn_to_real_speed((int16_t)(((uint16_t)rx_buff[2]) << 8 | rx_buff[3]));
         break;
      case 1:
         chassis.angle_rf_rcv = math::turn_to_real_angle((int16_t)(((uint16_t)rx_buff[0]) << 8 | rx_buff[1]));
         chassis.speed_rf_rcv = math::turn_to_real_speed((int16_t)(((uint16_t)rx_buff[2]) << 8 | rx_buff[3]));
         break;
      case 2:
         chassis.angle_lb_rcv = math::turn_to_real_angle((int16_t)(((uint16_t)rx_buff[0]) << 8 | rx_buff[1]));
         chassis.speed_lb_rcv = math::turn_to_real_speed((int16_t)(((uint16_t)rx_buff[2]) << 8 | rx_buff[3]));
         break;
      case 3:
         chassis.angle_rb_rcv = math::turn_to_real_angle((int16_t)(((uint16_t)rx_buff[0]) << 8 | rx_buff[1]));
         chassis.speed_rb_rcv = math::turn_to_real_speed((int16_t)(((uint16_t)rx_buff[2]) << 8 | rx_buff[3]));
         break;
      case 4:
         gimbal.angle_rcv = math::turn_to_real_angle((int16_t)(((uint16_t)rx_buff[0]) << 8 | rx_buff[1]));
         gimbal.speed_rcv = math::turn_to_real_speed((int16_t)(((uint16_t)rx_buff[2]) << 8 | rx_buff[3]));
         break;
      }
   }
}
